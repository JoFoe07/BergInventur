...
// Änderung notwendig da ich CRUD in das Modul gesetzt habe. Mit CRUD im Store funktinierte das Löschen nicht mehr
// http://www.sencha.com/forum/showthread.php?269790-How-to-delete-with-CRUD     
// 

//console.error(this.getList().getStore()._storeId);        	
if (this.getList().getStore()._storeId == "kundenterminestore") {
	this.getList().getStore().getProxy()._extraParams.id_termin = record.get('id');
}
else //(this.getList().getStore()._storeId == "Events_loesch") {
	{this.getList().getStore().getProxy()._extraParams.id_events = record.data.id_events;

};

							// Zeile hinzugefügt		
                    this.getList().getStore().remove(record);
					this.getList().getStore().sync(); 																// Zeile hinzugefügt da delete nicht aufgerufen wurde
