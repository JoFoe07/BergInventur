...
//if (!isset($_REQUEST["action"])) {
	...
	if (isset($_REQUEST["keyword"]))
		...
		$sql = "select [Pick Batch Identifier Id],[Name],[Code],lower([Name]) suchcode from [".$NAV."].[dbo].[".$Firma.'$'."Whse_ Employee Logistic]  WAL WITH (NOLOCK)\n"
			. "   where  \n"
			. "State=0 order by [Name]";		
...
$anzahl = sqlsrv_num_rows( $dbresult);
$result["total"] = $anzahl;
...
if (sqlsrv_num_rows ($dbresult) > 0) {
	...
	array_push($result["akiv"],array(
				"Code"=>$row['Code'],
				"Name"=>utf8_encode(addslashes((string)$row["Name"])),
				"PickBatchIdentifierId"=>$row['Pick Batch Identifier Id'],
				"suchcode"=>utf8_encode(addslashes((string)$row["suchcode"])),
				));
